...
async def start_button(message: types.Message):
    kb = [[types.KeyboardButton(text="Хочу посмотреть фильм")]]
...
async def genre_handler(callback: types.CallbackQuery, callback_data: Genre):
    ...
    await callback.message.answer(
        f"Вы выбрали жанр {genre}. Выберите год.", reply_markup=builder.as_markup()
    )
...
async def year_handler(callback: types.CallbackQuery, callback_data: Year):
    await callback.message.answer("oi")
    ...
    if not about_films:
        await callback.message.answer(
            "Такого сочетания нет. Выберите другие года и жанр."
        )
